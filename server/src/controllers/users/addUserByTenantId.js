...
/*
 * Input:  req.body { name, mobile, email, password, roleIds: [...] }
...
const addUserByTenantId = async (req, res, next) => {
  ...
  const { name, mobile, email, password, roleIds } = req.body;
  ...
  const user = await addUserByTenantIdService(tenantId, name, mobile, email, passwordHash, roleIds);
