...
## **Prerequisites:** You need to collect your ``PostgreSQL DBMS username, password, hostname``.

## **There are 2 ways to setup the project:**
### **1. Follow this automated setup step:** Just run ``npm install`` from the root. Everything goes in the flow.
...
#### iv. Next the DB needs the schemas or tables to create. Run ``npx prisma db push`` from server/ folder.
